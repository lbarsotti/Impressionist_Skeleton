...
public class ImpressionistView extends View {
    ...
    public static float rando(float min, float max){
    ...
    public boolean onTouchEvent(MotionEvent motionEvent){
        ...
        try {
            ...
            if (_brushType == BrushType.Random){
                drawX = rando(0, width);
                drawY = rando(0, height);

                pixel = otherBitmap.getPixel((int) drawX, (int) drawY);
                _paint.setColor(pixel);
            }
            else {
                pixel = otherBitmap.getPixel((int) touchX, (int) touchY);
                _paint.setColor(pixel);
            }
            ...
            if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                if (_brushType == BrushType.Circle)
                    ...
                    _offScreenCanvas.drawRect(touchX - 15, touchY + 15, touchX + 15, touchY - 15, _paint);
                else if (_brushType == BrushType.Line)
                    _offScreenCanvas.drawLine(touchX, touchY-15, touchX, touchY+15, _paint);
                else if (_brushType == BrushType.Random)
                    _offScreenCanvas.drawCircle(drawX, drawY, 15, _paint);
